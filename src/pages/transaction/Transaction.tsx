...
const Transaction: React.FC = () => {
  ...
  const [transactionType, setTransactionType] = useState<"All" | "Fiat" | "Crypto">("All");
  const [selectedTimeRange, setSelectedTimeRange] = useState("All Time");
  const [selectedAction, setSelectedAction] = useState<"All" | "Send" | "Fund" | "Convert" | "Withdraw" | "P2P" | "Bill Payments">("All");
  ...
  const actionButtons: ("All" | "Send" | "Fund" | "Convert" | "Withdraw" | "P2P" | "Bill Payments")[] = ["All", "Send", "Fund", "Convert", "Withdraw", "P2P", "Bill Payments"];
  ...
  const getTransactions = () => {
    ...
    if (transactionType === "Crypto") {
      ...
      switch (selectedAction) {
        case "All":
          // Combine all transaction types when "All" is selected
          transactions = [
            ...sendTransactions.map(t => ({ ...t, category: 'Fiat', transactionType: 'Send' })),
            ...fundTransactions.map(t => ({ ...t, category: 'Fiat', transactionType: 'Fund' })),
            ...convertTransactions.map(t => ({ ...t, category: 'Fiat', transactionType: 'Convert' })),
            ...withdrawTransactions.map(t => ({ ...t, category: 'Fiat', transactionType: 'Withdraw' })),
            ...p2pTransactions.map(t => ({ ...t, category: 'Fiat', transactionType: 'P2P' })),
            ...billPaymentsTransactions.map(t => ({ ...t, category: 'Fiat', transactionType: 'Bill Payments' }))
          ];
          // If transactionType is "All", also include crypto transactions
          if (transactionType === "All") {
            transactions = [
              ...transactions,
              ...cryptoTransactions.map(t => ({ ...t, category: 'Crypto', transactionType: 'Deposit' }))
            ];
          }
          break;
        case "Send":
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [selectedAction, transactionType, selectedStatus, selectedCountry, searchQuery]);

  // Calculate pagination for all filters
  ...
  const handleSelectAll = () => {
    const currentPageIndices = transactions.map((_, index) => (startIndex + index).toString());
    const allSelected = currentPageIndices.every(idx => selectedTransactions.has(idx));
    
    if (allSelected && currentPageIndices.length > 0) {
      setSelectedTransactions(prev => {
        const newSet = new Set(prev);
        currentPageIndices.forEach(idx => newSet.delete(idx));
        return newSet;
      });
    } else {
      setSelectedTransactions(prev => {
        const newSet = new Set(prev);
        currentPageIndices.forEach(idx => newSet.add(idx));
        return newSet;
      });
    }
  };

  const handleSelectTransaction = (localIndex: number) => {
      ...
      <div className="mb-6 flex items-center flex-wrap" style={{ paddingRight: '32px' }}>
        ...
        {selectedAction !== 'Convert' && selectedAction !== 'P2P' && selectedAction !== 'Bill Payments' && transactionType !== 'Crypto' && (
          <div className="relative" style={{ marginLeft: '15px' }}>
            ...
            >
              {selectedAction === 'All' ? 'Types' : ((selectedTxType === "Tx Type" || selectedTxType === "Route") ? getTxTypeLabel() : selectedTxType)}
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <polyline points="6 9 12 15 18 9"></polyline>
              </svg>
            </button>
            {showTxTypeDropdown && (
              <>
                ...
                `}</style>
                <div ref={txTypeDropdownRef} className="absolute top-full mt-2 z-50 tx-type-dropdown" style={{
                  width: selectedAction === 'All' ? '400px' : '105px',
                  left: selectedAction === 'All' ? '50%' : '0',
                  transform: selectedAction === 'All' ? 'translateX(-50%)' : 'none',
                  borderRadius: '10px',
                  backgroundColor: '#020C19B2',
                  boxShadow: '5px 5px 15px 0px #00000040',
                  backdropFilter: 'blur(4px)',
                  padding: selectedAction === 'All' ? '20px' : '4px 0',
                  overflow: 'hidden'
                }}>
                  {selectedAction === 'All' ? (
                    <>
                      ...
                      <div style={{ display: 'flex', gap: '12px' }}>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ 
                            ...
                            {["All Routes", "Bank Transfer", "Rhinox ID", "Mobile money"].map((option) => (
                              <button
                                key={option}
                                onClick={() => {
                                ...
                                className={`w-full text-left py-2 text-sm font-normal transition-colors hover:bg-white/5 ${selectedTxType === option ? 'bg-white/10' : ''}`}
                                style={{
                                  fontFamily: 'SF Pro, -apple-system, BlinkMacSystemFont, sans-serif',
                                  color: 'white',
                                  fontSize: '10px',
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ 
                            ...
                            {["All Routes", "Yellow Card", "Mobile Money", "Crypto", "Conversion", "P2P"].map((option) => (
                              <button
                                key={option}
                                onClick={() => {
                                  setSelectedTxType(option);
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ 
                            ...
                            {["All Routes", "Bank Transfer", "Mobile Money"].map((option) => (
                              <button
                                key={option}
                                onClick={() => {
                                  setSelectedTxType(option);
      ...
      <div>
        ...
        >
          ...
          <div style={{ width: '100%', borderBottomLeftRadius: '20px', borderBottomRightRadius: '20px', overflow: 'hidden' }}>
                        ...
                        <>
                          <td
                            ...
                            style={{
                              ...
                              verticalAlign: 'middle',
                              paddingLeft: selectedAction === 'All' ? '24px' : '0px',
                              paddingRight: selectedAction === 'All' ? '102px' : '62px'
                            }}
                          >
                            {transaction.amount}
                          </td>
            ...
            {/* Pagination - Show for all filters */}
            {allTransactions.length > 0 && (
              <div style={{
                ...
                <div style={{
                  ...
                  Showing {startIndex + 1} - {Math.min(endIndex, allTransactions.length)} of {allTransactions.length} {selectedAction === 'All' ? 'Users' : 'Transactions'}
                </div>
                <div style={{
                  display: 'flex',
                  alignItems: 'center',
